...
const IntegrationSlide = () => {
    const t = useTranslations();
    const logos = ['Slack', 'G-Suite', 'Salesforce', 'Jira', 'Hubspot'];
    return (
        <Slide>
            ...
            <div className="mt-12 flex flex-wrap justify-center items-center gap-8 md:gap-12 animate-fade-in-up" style={{ animationDelay: '0.6s' }}>
                {logos.map(logo => (
                    <div key={logo} className="h-12 flex items-center justify-center p-2" title={logo}>
                       <img src={`https://via.placeholder.com/140x50/FFFFFF/8B949E.png?text=${logo}`} alt={`${logo} Logo`} className="h-8 md:h-10 opacity-70 hover:opacity-100 transition-opacity" />
                    </div>
                ))}
            </div>
        </Slide>
